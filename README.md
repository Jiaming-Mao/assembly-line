...
- **文本渲染**：支持标题、副标题和自定义文本块，包含字体、颜色、对齐、阴影等样式
- **批量处理**：通过 CSV 文件批量生成封面
- **实时预览**：生成前可预览效果

### 界面功能

- **表单模式**：手动输入标题、副标题、背景图、截图等信息
...
CSV 文件建议使用 **按 key 显式映射** 的新格式（列名不区分大小写）。
...
| `text.<textKey>` | 填充任意文本块（按模板 `texts[].key`） | `text.title`、`text.text-2` |
| `slot.<slotKey>` | 填充任意图片插槽（按模板 `slots[].key`） | `slot.ui-1`、`slot.background` |

**示例（推荐）：**

```csv
template_key,output_name,background_path,text.title,text.subtitle,text.text-2,slot.background,slot.glass,slot.ui-1
left-right-green,case-001.png,,一张表管公司,让业务流转起来,（可选的额外文案）,/abs/bg.png,/abs/glass.png,/abs/ui.png
ai-todo-cover,case-002.png,,,,/abs/bg.png,/abs/glass.png,/abs/ui.png
```
...
| 列名 | 说明 | 示例 |
|------|------|------|
| `title` | 标题（等价于 `text.title` 的回退来源） | "我的应用" |
| `subtitle` | 副标题（等价于 `text.subtitle` 的回退来源） | "让生活更美好" |
| `background` | 背景图片路径（等价于 `background_path`） | "/path/to/bg.png" |
| `screenshots` | 截图路径（逗号或竖线分隔），**仅在未提供任何 `slot.*` 列时**按模板 slot 顺序依次填充 | "/path/to/1.png,/path/to/2.png" |
| `template` 或 `layout` | 模板键名（等价于 `template_key`） | "default" |
| `output` 或 `output_name` | 输出文件名（等价于 `output_name`） | "cover-001.png" |

**优先级规则：**
- **Text**：如果提供 `text.<key>` 列，优先使用；否则 `title/subtitle` 仅作为 `title/subtitle` 两个 key 的回退。\n+- **Slot**：如果提供任意 `slot.*` 列，则只按 `slot.<key>` 填充；否则才使用 `screenshots` 做顺序填充。
...
### Q: CSV 批量生成时，如何指定多个截图？

A: 在 CSV 的 `screenshots` 列中，用逗号或竖线分隔多个路径：
```
/path/to/1.png,/path/to/2.png
```
或
```
/path/to/1.png|/path/to/2.png
